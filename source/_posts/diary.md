...
组里来了一台微光相机，配合ai一些简单的curl指令也是发现是onvif协议，轻松看到画面，有一个有点理解但又没那么理解的事情，就是这些用网线通信的设备，基本上都是设置静态ip，万一有的设备在.0.x网段，有的设备在.1.x网段，如果主机只有一个网口，应该如何两个都能访问，查了一下，原来可以设置不同网段的两个ip给同一个网卡，反之也是局域网通信，没什么问题，还真是
...
折腾了一下PayPal尝试白嫖gpt的第一个月，失败，看别人说是五月加强风控了，尝试Apple土区账号，已成功前置步骤，还没买，我是看的b站的教程，如果转区的时候没有None，可以进[这里](https://account.apple.com/account/manage/section/information)开全局代理换区，我这样就出现none了
...
> 确实可以，爽用，[Proxifier激活](https://linux.do/t/topic/1545330)

了解了一下uv，浅浅体验了一下确实还不错，不过到底该怎么看论文呢（已发疯啊啊啊啊啊）六级也搁置挺久了其实

VS Code的右下角自带了一个**设置同步**的按钮，可以通过github账号进行同步，还挺好用的，今天获得了流传下来的电脑，喜欢用win10的磁铁故重装系统了，不过这个诡异的8851BE网卡网上下了几个都不能用，无可奈何试了下驱动精灵甚至没有识别到这张网卡，然后试了试驱动总裁，换版本多试了几个总算是行了

>  不过诡异的是这个软件的下载链接竟然是百度网盘，更离谱的是蓝牙显示正常但就是扫描不出新设备，有亿点点沉默🤐
...
最近一直在干项目，后期是做实验，感觉会比前期整体轻松一点，因为有时候做不了实验或者不需要我们的时候，是纯粹的放松时间，但是我是真的有亿点点晕船xD

看到频道有一位uu说电脑用不了代理，自告奋勇排查了半天，最后想起来他提到他是用PPPoE拨号上网的，我就让他用手usb数据共享来进行测试，没想到还真是这个问题，搜了一下还是有点意思的。一般而言只需要把PPPoE连接的名字改成英文即可，算是Windows的锅，在拨号名称为中文的时候代理设置就是有问题，在是英文的情况下，拨号的代理设置和正常dhcp的代理设置不是同一个注册表，所以还有一种情况是代理软件没有适配，clash party在2024年8月应该就已经修复了软件适配的[问题](https://github.com/mihomo-party-org/clash-party/issues/47)，其他的软件应该是也都没问题的
...
朋友们在玩文明6，总吐槽服务器卡顿，于是想着自建个中转服务器试试，一番观察选择了**[n2n](https://github.com/ntop/n2n)**这个项目，确实有些年代了，不过应该还是没什么问题的

* **服务器端**部署在公网云服务器上，在[n2n Releases](https://github.com/ntop/n2n/releases)中下载最新的安装包安装

  > 千万不要使用包管理器如apt直接安装，那个版本老掉牙了
  ...
  从这里可以看看版本和使用参数，**确保版本在`v3.0`及以上**，其中`-f`可以指定只在前台运行，没有的话就会后台运行

  ```
  # 在终端前台测试，按ctrl-c即可停止
  supernode -p 4210 -a 192.168.4.0-192.168.4.0/24 -c /root/n2n/community -f
  ```

  > 这个-a分配的ip范围写法还是有点迷惑的，默认值为`-a 10.128.255.0-10.255.255.0/24`，一般来说都写的是某ip-某ip或者使用/24，/16来指定范围，但他的写法是二者的结合，比如`-a 192.168.0.0-192.168.255.0/24`其实是指`192.168.0.x`、`192.168.1.x`、……一直到`192.168.255.x`

  确保端口都正常开放和可连接后（可用`telnet`测试），可以把所有的参数写入配置文件，注意是一行一个参数

  ```
  # sudo cat /etc/n2n/supernode.conf
  -p 4210 
  -a 192.168.4.0-192.168.4.0/24 
  -c /etc/n2n/community
  ```

  `-c`所带的文件里面的内容决定了小组名称，你可以简单的理解为密码，请尽量的复杂

  ```
  # sudo cat /etc/n2n/community 
  e136f475-5354-47b1
  ```
...
* **客户端**推荐使用[EasyN2N（小黄鸭）](https://bugxia.com/357.html)，非常清晰易懂，确实称得上是Easy

  推荐在`测试工具`中设置下面几个内容

  * ping测试→一键关闭防火墙
  * 联机相关→方法1：WinIPBroadcast
  * 联机相关→方法2：网卡优先级（跃点数），把虚拟网卡的跃迁点改为最低

克隆https的仓库时，只需要在clash设置里面找到类似这样的指令，在终端中执行一下即可
